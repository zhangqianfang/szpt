...
<script>
  ...
  export default {
    data() {
      return {
        titleName: { name: "教学经费在总支出占比" },
        titleName1: { name: "" },
        headerData: { name: "教学经费在总支出占比" },
        totalNum: '',
        xData: [],
        yData: [],
        paginationShow: true,
    ...
    mounted: function () {
      http.api({
        url: HTTP_ROOT + '/api/jap/ExpendTotal_Bar',
        params: getCommonParams(),
        successCallback: function (data) {
          let {xData, yData} = this.getYdataByAdd(data.workTableDate,'T_SCHOOL_CITY','TREPORTDATA18_VALUE4','TREPORTDATA18_VALUE5','TREPORTDATA18_VALUE1');
    ...
    methods: {
      ...
      getYdataByAdd: function (data,xcolumnName,addColumn,addedColumn,divideColumn) {
        ...
        allInfo.forEach(p => {
          ...
          yData.push(((parseInt(addNum)+parseInt(addedNum)*100)/divideNum).toFixed(2));
        })
        return {xData,yData}
      },
      getNeedGridInfoByAdd: function (data,columnName1,columnName2,addColumn,addedColumn,dividendColumn,percent=100) {
        let allInfo = JSON.parse(data);
        let needGridList = []
        allInfo.forEach(p => {
          let needGridInfo = {};
          ...
          needGridInfo.value = ((addNum+addedNum) * 100/dividend).toFixed(2);
      ...
      getGridInfo: function (cityName,page={currentPage: 1}) {
        ...
        http.api({
          url: HTTP_ROOT + '/api/jap/ExpendTotal_Grid',
          params: getGridParams("T_SCHOOL_CITY",cityName,page.currentPage),
          successCallback: function (data) {
            this.page.total = parseInt(JSON.parse(data.pageInfo)[0][0].value)
            this.tableList.tbody = this.getNeedGridInfoByAdd(data.workTableDate,"TREPORTDATA1_VALUE1","TREPORTDATA1_VALUE2","TREPORTDATA18_VALUE4",'TREPORTDATA18_VALUE5','TREPORTDATA18_VALUE1')
