...
<script>
  ...
  export default {
    data() {
      return {
        titleName: {name: "本地专业平均学生数"},
        titleName1: {name: ""},
        headerData: {name: "本地专业平均学生数"},
        totalNum: '',
        xData: [],
        yData: [],
        tableList: {
          thead: ["学校代码", "学校名称", "专业生均数"],
          tbody: []
        },
      };
    },
    mounted: function () {
      http.api({
        url: HTTP_ROOT + '/api/jap/majorCon_localMajorStudentNumAvg_city',
        params: this.getParams("T_SCHOOL_PROVINCE", defaultValue.provinceName),
        successCallback: function (data) {
          let {xData, yData} = getXyData(data.workTableDate, 'T_SCHOOL_CITY', 'TREPORTDATA29_VALUE10');
          this.xData = xData;
      ...
      http.api({
        ...
        params: this.getParams("T_SCHOOL_PROVINCE", defaultValue.provinceName),
        successCallback: function (data) {
          let {xData, yData} = getXyData(data.workTableDate, 'T_SCHOOL_PROVINCE', 'TREPORTDATA29_VALUE10');
          this.totalNum = parseFloat(yData[0]).toFixed(2);
        }.bind(this)
      })
    },
    methods: {
      ...
      getGridInfo: function (cityName) {
        ...
        http.api({
          url: HTTP_ROOT + '/api/jap/majorCon_localMajorStudentNumAvg_school',
          params: {
            "limit": 0,
            "filters": [
              {
          ...
          successCallback: function (data) {
            var array = getNeedGridInfo(data.workTableDate, "T_SCHOOL_SCHOOL_CODE", "T_SCHOOL_SCHOOL_NAME", "TREPORTDATA29_VALUE10");
            for (var i = 0; i < array.length; i++) {
              array[i].value = parseFloat(array[i].value).toFixed(2);
            }
