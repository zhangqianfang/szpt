  ...
  <div id="localKeyMajorNumDiv">
    ...
    <div class="receipts_left">
      ...
      <div class="work_map">
        ...
        <BarCharts :x-data="xData" :y-data="yData" echart-id="localKeyMajorNumAvgEcharts"
...
<script>
  ...
  export default {
    data() {
      return {
        titleName: {name: "本地重点建设专业平均数"},
        titleName1: {name: ""},
        headerData: {name: "本地重点建设专业平均数"},
        totalNum: '',
        xData: [],
        yData: [],
        tableList: {
            ...
            "filters": [
              {
                ...
                "listFilter": defaultValue.stateKey + "," + defaultValue.provincePoint,
                "tableType": "1",
                "inOrNot": "0"
              },
            ]
    ...
    mounted: function () {
      http.api({
        ...
        successCallback: function (data) {
          let {xData, yData} = getXyData(data.workTableDate, 'T_SCHOOL_CITY', 'COUNT1');
          http.api({
              ...
              "filters": [
                {
                  ...
                  "tableName": "",
                  "tableColumn": "T_SCHOOL_CITY",
                  "listFilter": JSON.stringify(xData),
                  "tableType": "1",
                  "inOrNot": "0"
                }
              ]
            ...
            successCallback: function (data) {
              let yyData = [];
              let object2 = getXyData(data.workTableDate, 'T_SCHOOL_CITY', 'T_SCHOOL_ID');
              for (var i = 0; i < xData.length; i++) {
                yyData.push(0);
                for (var j = 0; j < object2.xData.length; j++) {
                  if (xData[i] == object2.xData[j]) {
                    if (object2.yData[j] > 0) {
                      yyData[i] = (yData[i] / object2.yData[j]).toFixed(2);
                    }
                    break;
                  }
                }
              ...
              this.xData = xData;
              this.yData = yyData;
              this.totalNum = (yData.reduce((acc, value) => parseInt(acc) + parseInt(value), 0) / object2.yData.reduce((acc, value) => parseInt(acc) + parseInt(value), 0)).toFixed(2);
