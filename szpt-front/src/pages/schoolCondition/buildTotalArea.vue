...
<script>
  ...
  export default {
    data() {
      return {
        titleName: { name: "建筑总面积" },
        titleName1: { name: "" },
        headerData: { name: "建筑总面积" },
        totalNum: '',
        xData: [],
        yData: [],
        paginationShow: true,
    ...
    methods: {
      ...
      getYdataByAdd: function (data,xcolumnName,addColumn,addColumn2,addColumn3,addColumn4,addColumn5,addedColumn,divideColumn) {
        ...
        allInfo.forEach(p => {
          ...
          yData.push(((parseInt(addNum)+parseInt(addNum2)+parseInt(addNum3)+parseInt(addNum4)+parseInt(addNum5)+parseInt(addedNum))/10000).toFixed(2));
        })
        return {xData,yData}
      },
