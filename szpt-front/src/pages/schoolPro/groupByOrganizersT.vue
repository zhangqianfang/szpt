...
export default {
  ...
  data() {
    ...
    barChartOption.xAxis.data = ['教育部门','地方企业','政府','民办','其他部门']
  ...
  methods: {
    ...
    initChart(){
      this._defaultQuery('/schoolPro_groupByOrgnizersT_echart', {limit:"100"}, (data)=>{
        let array = this.japUtils.getMapIntValue(data.workTableDate, ['教育部门','地方企业','政府','民办','其他部门']);
    ...
    initTable(paramName, page = {pageNo:1,limit:this.page.pageSize}){
      let option = {
        limit: page.limit,
        filters: [{
          tableColumn: "TREPORTDATA1_VALUE7",
      ...
      this._defaultQuery('/schoolPro_getSchoolByCategoryOrOrganizer', option, (data)=>{
        ...
        this.tableList.tbody = this.japUtils.getTableValue(data.workTableDate, [0,1,2,4], ['name','statistical','value','value2']);
      });
    },
