<!--按专业技术职务分布的专任教师总数-->
<template>
  <div id="occupationFullTimeTeacherNumDiv">
    ...
    <div class="receipts_left">
      ...
      <div class="work_map">
        ...
        <BarCharts :x-data="xData" :y-data="yData" echart-id="occupationFullTimeTeacherNumEcharts"
...
<script>
  ...
  export default {
    data() {
      return {
        titleName: {name: "按专业技术职务分布的专任教师总数"},
        titleName1: {name: ""},
        headerData: {name: "按专业技术职务分布的专任教师总数"},
        totalNum: '',
        xData: [],
        yData: [],
        tableList: {
          thead: ["学校代码", "学校名称", "教师总数"],
          tbody: []
        },
      };
    },
    mounted: function () {
      http.api({
        url: HTTP_ROOT + '/api/jap/teacher_occupationFullTimeTeacherNum_area',
        params: this.getParams("T_SCHOOL_PROVINCE", defaultValue.provinceName),
        successCallback: function (data) {
          let {xData, yData} = getXyData2(data.workTableDate, 'TREPORTDATA19_VALUE13', 'COUNT1');
          var yyData = [];
          step1: for (var i = 0; i < defaultValue.seniorTitleSort.length; i++) {
            for (var j = 0; j < xData.length; j++) {
              if (xData[j] == defaultValue.seniorTitleSort[i]) {
                yyData.push(yData[j]);
                continue step1;
              }
            }
          ...
          this.xData = defaultValue.seniorTitleSort;
          this.yData = yyData;
    ...
    methods: {
      ...
      getGridInfo: function (name) {
        this.titleName1.name = name;
        http.api({
          url: HTTP_ROOT + '/api/jap/teacher_occupationFullTimeTeacherNum_school',
          params: {
            "limit": 0,
            "filters": [
              {
...
<style lang="scss" scoped>
  #occupationFullTimeTeacherNumDiv {
    .receipts_left {
