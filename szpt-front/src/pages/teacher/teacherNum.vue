  ...
  <div id="teacherNumDiv">
    ...
    <div class="receipts_left">
      ...
      <div class="work_map">
        ...
        <BarCharts :x-data="xData" :y-data="yData" echart-id="teacherNumEcharts"
...
<script>
  ...
  export default {
    data() {
      return {
        titleName: {name: "教职工总数"},
        titleName1: {name: ""},
        headerData: {name: "教职工总数"},
        totalNum: '',
        xData: [],
        yData: [],
        tableList: {
          thead: ["学校代码", "学校名称", "教职工总数"],
          tbody: []
        },
      };
    },
    mounted: function () {
      http.api({
        url: HTTP_ROOT + '/api/jap/teacher_teacherNum_city',
        params: this.getParams("T_SCHOOL_PROVINCE", defaultValue.provinceName),
        successCallback: function (data) {
          let {xData, yData} = this.getXyData(data.workTableDate, 'T_SCHOOL_CITY', ['TREPORTDATA6_VALUE6', 'TREPORTDATA6_VALUE7', 'TREPORTDATA6_VALUE9', 'TREPORTDATA6_VALUE10']);
          this.xData = xData;
          this.yData = yData;
          this.getGridInfo(xData[0]);
          this.totalNum = yData.reduce((acc, value) => acc + value, 0);
        }.bind(this)
      })
    },
    methods: {
      ...
      getXyData: function (data, xColumnName, dataArrar) {
        let allInfo = JSON.parse(data);
        let xData = [], yData = [], index = 0
        allInfo.forEach(p => {
          index++;
          p.forEach(x => {
            if (x.columnName === xColumnName)
              xData.push(x.value);
            if (dataArrar.indexOf(x.columnName) > -1) {
              if (yData.length < index) {
                yData.push(parseInt(x.value));
              } else {
                yData[index - 1] = yData[index - 1] + parseInt(x.value);
              }
            }
          })
        })
      ...
      getNeedGridInfo: function (data, columnName1, columnName2, dataArrar) {
        let allInfo = JSON.parse(data);
        let needGridList = []
        allInfo.forEach(p => {
          let needGridInfo = {};
          p.forEach(x => {
            ...
            if (dataArrar.indexOf(x.columnName) > -1) {
              if (null == needGridInfo.value) {
                needGridInfo.value = parseInt(x.value);
              } else {
                needGridInfo.value = needGridInfo.value + parseInt(x.value);
              }
            }
          })
      ...
      getGridInfo: function (cityName) {
        ...
        http.api({
          url: HTTP_ROOT + '/api/jap/teacher_teacherNum_school',
          params: {
            "limit": 0,
            "filters": [
              {
          ...
          successCallback: function (data) {
            var array = this.getNeedGridInfo(data.workTableDate, "T_SCHOOL_SCHOOL_CODE", "T_SCHOOL_SCHOOL_NAME", ['TREPORTDATA6_VALUE6', 'TREPORTDATA6_VALUE7', 'TREPORTDATA6_VALUE9', 'TREPORTDATA6_VALUE10'])
