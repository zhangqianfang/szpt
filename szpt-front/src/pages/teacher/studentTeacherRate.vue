  ...
  <div id="studentTeacherRateDiv">
    ...
    <div class="receipts_left">
      ...
      <div class="work_map">
        ...
        <BarCharts :x-data="xData" :y-data="yData" echart-id="studentTeacherRateEcharts"
...
<script>
  ...
  export default {
    data() {
      return {
        titleName: {name: "平均生师比"},
        titleName1: {name: ""},
        headerData: {name: "平均生师比"},
        totalNum: '',
        xData: [],
        yData: [],
        tableList: {
          thead: ["学校代码", "学校名称", "平均生师比"],
          tbody: []
        },
      };
    },
    mounted: function () {
      http.api({
        ...
        successCallback: function (data) {
          ...
          http.api({
            url: HTTP_ROOT + '/api/jap/teacher_studentTeacherRate_studentNum_city',
            params: this.getParams("T_SCHOOL_PROVINCE", defaultValue.provinceName),
            successCallback: function (data) {
              var data2 = getXyData2(data.workTableDate, 'T_SCHOOL_CITY', 'TREPORTDATA5_VALUE2');
              let yyData = [];
              step1: for (var i = 0; i < data1.xData.length; i++) {
                for (var j = 0; j < data2.xData.length; j++) {
                  if (data1.xData[i] == data2.xData[j]) {
                    if (data1.yData[i] > 0) {
                      yyData.push((data2.yData[j] / data1.yData[i]).toFixed(2));
                    } else {
                      yyData.push(0);
                    }
                    continue step1;
              ...
              this.totalNum = (data2.yData.reduce((acc, value) => acc + parseInt(value), 0) / data1.yData.reduce((acc, value) => acc + parseInt(value), 0)).toFixed(2);
            }.bind(this)
          })
        }.bind(this)
      })
      ...
      getGridInfo: function (cityName) {
        ...
        http.api({
          ...
          successCallback: function (data) {
            ...
            http.api({
              url: HTTP_ROOT + '/api/jap/teacher_studentTeacherRate_studentNum_school',
              params: {
                "limit": 0,
                "filters": [
                  {
              ...
              successCallback: function (data) {
                var array2 = getNeedGridInfo2(data.workTableDate, "T_SCHOOL_SCHOOL_CODE", "T_SCHOOL_SCHOOL_NAME", 'TREPORTDATA5_VALUE2')

                this.tableList.tbody = array1.reduce(function (acc, value) {
                  for (var i = 0; i < array2.length; i++) {
                    if (value.name == array2[i].name) {
                      if (value.value > 0) {
                        value.value = (array2[i].value / value.value).toFixed(2);
                      } else {
                        value.value = 0;
                      }
                      acc.push(value);
