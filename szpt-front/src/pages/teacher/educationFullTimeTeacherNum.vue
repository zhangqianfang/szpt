<!--按学历分布的专任教师总数-->
<template>
  <div id="educationFullTimeTeacherNumDiv">
    ...
    <div class="receipts_left">
      ...
      <div class="work_map">
        ...
        <BarCharts :x-data="xData" :y-data="yData" echart-id="educationFullTimeTeacherNumEcharts"
...
<script>
  ...
  export default {
    data() {
      return {
        titleName: {name: "按学历分布的专任教师总数"},
        titleName1: {name: ""},
        headerData: {name: "按学历分布的专任教师总数"},
        totalNum: '',
        xData: [],
        yData: [],
        tableList: {
    ...
    mounted: function () {
      http.api({
        url: HTTP_ROOT + '/api/jap/teacher_educationFullTimeTeacherNum_area',
        params: this.getParams("T_SCHOOL_PROVINCE", defaultValue.provinceName),
        successCallback: function (data) {
          let {xData, yData} = getXyData2(data.workTableDate, 'TREPORTDATA19_VALUE7', 'COUNT1');
          var yyData = [];
          step1: for (var i = 0; i < defaultValue.educationSort.length; i++) {
            for (var j = 0; j < xData.length; j++) {
              if (xData[j] == defaultValue.educationSort[i]) {
                yyData.push(yData[j]);
                continue step1;
              }
            }
          ...
          this.xData = defaultValue.educationSort;
          this.yData = yyData;
...
<style lang="scss" scoped>
  #educationFullTimeTeacherNumDiv {
    .receipts_left {
