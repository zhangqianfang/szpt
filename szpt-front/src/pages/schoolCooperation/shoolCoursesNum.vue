...
<script>
  ...
  import {getXyDataByDivision, getNeedGridInfo, getCommonParams, getGridParams} from '../../utils/common';
  import BarCharts from '../../components/barCharts';
  export default {
    data() {
      return {
        titleName: { name: '学校平均共同开发课程数' },
        titleName1: { name: '' },
        headerData: { name: '学校平均共同开发课程数' },
        totalNum: '',
        xData: [],
        yData: [],
        paginationShow: true,
    ...
    mounted: function () {
      http.api({
        url: HTTP_ROOT + '/api/jap/SchoolCompanyCooper_Bar',
        params: getCommonParams(),
        successCallback: function (data) {
          let {xData, yData} = getXyDataByDivision(data.workTableDate,'T_SCHOOL_CITY','TREPORTDATA35_VALUE7','T_SCHOOL_ID',1);
    ...
    methods: {
      ...
      getGridInfo: function (cityName,page={currentPage: 1}) {
        ...
        http.api({
          url: HTTP_ROOT + '/api/jap/SchoolCompanyCooper_Grid',
          params: getGridParams("T_SCHOOL_CITY",cityName,page.currentPage),
          successCallback: function (data) {
            this.page.total = parseInt(JSON.parse(data.pageInfo)[0][0].value)
            this.tableList.tbody = getNeedGridInfo(data.workTableDate,"TREPORTDATA1_VALUE2","TREPORTDATA1_VALUE1","TREPORTDATA35_VALUE7")
