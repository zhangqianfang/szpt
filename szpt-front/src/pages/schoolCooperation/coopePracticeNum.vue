...
<script>
  ...
  export default {
    data() {
      return {
        titleName: { name: "合作企业接受顶岗实习学生数" },
        titleName1: { name: "" },
        headerData: { name: "合作企业接受顶岗实习学生数" },
        totalNum: '',
        xData: [],
        yData: [],
        paginationShow: true,
    ...
    mounted: function () {
      http.api({
        url: HTTP_ROOT + '/api/jap/SchoolCompanyCooper_Bar_Last',
        params: getCommonParams(),
        successCallback: function (data) {
          let {xData, yData} = getXyData(data.workTableDate,'T_SCHOOL_CITY','TREPORTDATA35_VALUE10');
    ...
    methods: {
      ...
      getGridInfo: function (cityName,page={currentPage: 1}) {
        ...
        http.api({
          url: HTTP_ROOT + '/api/jap/SchoolCompanyCooper_Grid_Last',
          params: getGridParams("T_SCHOOL_CITY",cityName,page.currentPage),
          successCallback: function (data) {
            this.page.total = parseInt(JSON.parse(data.pageInfo)[0][0].value)
            this.tableList.tbody = getNeedGridInfo(data.workTableDate,"TREPORTDATA1_VALUE2","TREPORTDATA35_VALUE10","TREPORTDATA35_VALUE9")
