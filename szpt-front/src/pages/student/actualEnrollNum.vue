...
<script>
  ...
  export default {
    data() {
      return {
        titleName: { name: "当年实际招生数" },
        titleName1: { name: "" },
        headerData: { name: "当年实际招生数" },
        totalNum: '',
        xData: [],
        yData: [],
        tableList: {
    ...
    mounted: function () {
      http.api({
        url: HTTP_ROOT + '/api/jap/ActualEnrollNum_Bar',
        successCallback: function (data) {
          let {xData, yData} = getXyData(data.workTableDate,'T_SCHOOL_CITY','TREPORTDATA36_VALUE8');
          this.xData = xData;
          this.yData = yData;
    ...
    methods: {
      ...
      getGridInfo: function (cityName,page={currentPage: 1}) {
        ...
        http.api({
          url: HTTP_ROOT + '/api/jap/ActualEnrollNum_Grid',
          params: getGridParams("T_SCHOOL_CITY",cityName,page.currentPage),
          successCallback: function (data) {
            this.page.total = parseInt(JSON.parse(data.pageInfo)[0][0].value)
            this.tableList.tbody = getNeedGridInfo(data.workTableDate,"TREPORTDATA1_VALUE2","TREPORTDATA1_VALUE1","TREPORTDATA36_VALUE8")
