...
<script>
  ...
  export default {
    data() {
      return {
        titleName: { name: "学生获奖情况" },
        titleName1: { name: "" },
        headerData: { name: "学生获奖情况" },
        totalNum: '',
        xData: [],
        yData: [],
        tableList: {
    ...
    mounted: function () {
      http.api({
        url: HTTP_ROOT + '/api/jap/StudentAwardSituation',
        params: getCommonParams(),
        successCallback: function (data) {
          let {xData, yData} = getXyData(data.workTableDate,'TREPORTDATA51_VALUE4','TREPORTDATA51_VALUE2');
          this.xData = xData;
          this.yData = yData;
    ...
    methods: {
      ...
      getGridInfo: function (cityName,page={currentPage: 1}) {
        ...
        http.api({
          url: HTTP_ROOT + '/api/jap/StudentAwardSituationGrid',
          params: getGridParams("TREPORTDATA51_VALUE4",cityName, page.currentPage),
          successCallback: function (data) {
            this.page.total = parseInt(JSON.parse(data.pageInfo)[0][0].value)
            this.tableList.tbody = getNeedGridInfo(data.workTableDate,"TREPORTDATA1_VALUE2","TREPORTDATA1_VALUE1","TREPORTDATA51_VALUE2")
