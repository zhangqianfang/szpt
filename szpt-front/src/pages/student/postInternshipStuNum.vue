  ...
  <div id="allDaySchool">
    ...
    <div class="receipts_left">
      ...
      <div class="work_map">
        ...
        <BarCharts :x-data="xData" :y-data="yData" echart-id="incomeTotal" @chartsClick="chartsClick"></BarCharts>
      </div>
    </div>
...
<script>
  ...
  export default {
    data() {
      return {
        titleName: { name: "顶岗实习学生总数" },
        titleName1: { name: "" },
        headerData: { name: "顶岗实习学生总数" },
        totalNum: '',
        xData: [],
        yData: [],
        paginationShow: true,
    ...
    mounted: function () {
      http.api({
        url: HTTP_ROOT + '/api/jap/PostInternshipStuNum_Bar',
        params: getCommonParams(),
        successCallback: function (data) {
          let {xData, yData} = getXyData(data.workTableDate,'T_SCHOOL_CITY','TREPORTDATA34_VALUE12');
          this.getGridInfo(xData[0]);
          this.xData = xData;
          this.yData = yData;
          this.totalNum = yData.reduce((acc,value)=>parseFloat(acc)+parseFloat(value),0);
    ...
    methods: {
      ...
      getGridInfo: function (cityName,page={currentPage: 1}) {
        ...
        http.api({
          url: HTTP_ROOT + '/api/jap/PostInternshipStuNum_Grid',
          params: getGridParams("T_SCHOOL_CITY",cityName,page.currentPage),
          successCallback: function (data) {
            this.page.total = parseInt(JSON.parse(data.pageInfo)[0][0].value)
            this.tableList.tbody = getNeedGridInfo(data.workTableDate,"TREPORTDATA1_VALUE1","TREPORTDATA1_VALUE2","TREPORTDATA34_VALUE12")
