  ...
  <div id="notAllDaySchool">
    ...
    <div class="receipts_left">
      ...
      <div class="work_map">
        ...
        <BarCharts :x-data="xData" :y-data="yData" echart-id="notAllDay" @chartsClick="chartsClick"></BarCharts>
      </div>
    </div>
...
<script>
  ...
  import {getXyData, getNeedGridInfo, getGridParams, getCommonParams} from '../../utils/common';
  ...
  export default {
    data() {
      return {
        titleName: { name: "非全日制中职学历教育在校生总数" },
        titleName1: { name: "" },
        headerData: { name: "非全日制中职学历教育在校生总数" },
        totalNum: '',
        xData: [],
        yData: [],
        tableList: {
    ...
    mounted: function () {
      http.api({
        url: HTTP_ROOT + '/api/jap/SchoolStudentNum_Bar',
        params: getCommonParams(),
        successCallback: function (data) {
          let {xData, yData} = getXyData(data.workTableDate,'T_REGION_CITY_NAME','TREPORTDATA5_VALUE16');
          this.xData = xData;
          this.yData = yData;
    ...
    methods: {
      ...
      getGridInfo: function (cityName,page={currentPage: 1}) {
        ...
        http.api({
          url: HTTP_ROOT + '/api/jap/SchoolStudentNum_Grid',
          params: getGridParams("T_SCHOOL_CITY",cityName,page.currentPage),
          successCallback: function (data) {
            this.page.total = parseInt(JSON.parse(data.pageInfo)[0][0].value)
            this.tableList.tbody = getNeedGridInfo(data.workTableDate,"TREPORTDATA1_VALUE2","TREPORTDATA1_VALUE1","TREPORTDATA5_VALUE16")
