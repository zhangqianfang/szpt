...
<script>
  ...
  export default {
    data() {
      return {
        titleName: { name: "享受奖助学金人数" },
        titleName1: { name: "" },
        headerData: { name: "享受奖助学金人数" },
        totalNum: '',
        xData: [],
        yData: [],
        paginationShow: true,
    ...
    mounted: function () {
      http.api({
        url: HTTP_ROOT + '/api/jap/EnjoyScholarship_Bar',
        params: getCommonParams(),
        successCallback: function (data) {
          let {xData, yData} = getXyData(data.workTableDate,'T_SCHOOL_CITY','TREPORTDATA46_VALUE5');
    ...
    methods: {
      ...
      getGridInfo: function (cityName,page={currentPage: 1}) {
        ...
        http.api({
          url: HTTP_ROOT + '/api/jap/EnjoyScholarship_Grid',
          params: getGridParams("T_SCHOOL_CITY",cityName,page.currentPage),
          successCallback: function (data) {
            this.page.total = parseInt(JSON.parse(data.pageInfo)[0][0].value)
            this.tableList.tbody = getNeedGridInfo(data.workTableDate,"TREPORTDATA1_VALUE1","TREPORTDATA1_VALUE2","TREPORTDATA46_VALUE5")
