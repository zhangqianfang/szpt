...
export default {
  ...
  data() {
    ...
    return {
      titleName: { name: "面向社会开展职业技能鉴定的情况" },
  ...
  methods: {
    ...
    initTotalCount(){
      this._defaultQuery('/socialWork_areaDataCollect_table1', {limit:"100"}, (data)=>{
        let array = this.japUtils.getManyIntSum(data.workTableDate, [1,2]);
        this.totalCount = array[0] + array[1];
      });
    },
    initChart(){
      this._defaultQuery('/socialWork_developPSI_echart', {limit:"100"}, (data)=>{
        [this.barChart.xAxis.data, this.barChart.opinionData] = this.japUtils.getEchartSumValue(data.workTableDate, undefined, [0,1,2]);
    ...
    initTable(cityName, page = {pageNo:1,limit:this.page.pageSize}){
      ...
      this._defaultQuery('/socialWork_developPSI_detail', option, (data)=>{
        this.page.total = parseInt(JSON.parse(data.pageInfo)[0][0].value);
        this.tableList.tbody = this.japUtils.getTableValue(data.workTableDate, [0,1,2], ['v1','v2','v3'],{v4:[3,4]});
      });
    },
